...
class Dense(Layer):
    ...
    def learn(self, input_previous_layer, weighted_errors_next_layer):
        errors=weighted_errors_next_layer*(self.function.derivative(self.weights@input_previous_layer + self.biases))
        weighted_errors=np.transpose(self.weights)@errors
        self.weights-=self.learning_rate*np.array(errors*node for node in input_previous_layer)
        self.biases-=self.learning_rate*errors
        return weighted_errors


class Convolutional(Layer):
    def __init__(self, dim_input, function, filter_size, filter_number, stride=1, weights=None, biases=None, learning_rate=1):
